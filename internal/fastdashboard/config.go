...
func recursiveParseYAMLIncludes(mainFilePath string, includes map[string]struct{}, depth int) ([]byte, map[string]struct{}, error) {
	...
	mainFileContents = configIncludePattern.ReplaceAllFunc(mainFileContents, func(match []byte) []byte {
		...
		fileContents, _, err := recursiveParseYAMLIncludes(includeFilePath, includes, depth+1)
		if err != nil {
			includesLastErr = err
			return nil
		}
...
func configFilesWatcher(
	...
	parseAndCompareBeforeCallback := func() {
		...
		watchedSet := make(map[string]struct{}, len(currentIncludes)+1)
		for k := range currentIncludes {
			watchedSet[k] = struct{}{}
		}
		watchedSet[mainFileAbsPath] = struct{}{}

		mu.Lock()
		defer mu.Unlock()

		if !maps.Equal(watchedSet, lastIncludes) {
			updateWatchedFiles(lastIncludes, watchedSet)
			lastIncludes = watchedSet
		}
	...
	for i := range config.Pages {
		...
		if page.DesktopNavigationWidth != "" {
			if page.DesktopNavigationWidth != "wide" && page.DesktopNavigationWidth != "slim" && page.DesktopNavigationWidth != "default" {
				return fmt.Errorf("page %d: desktop-navigation-width must be wide, slim, or default", i+1)
			}
		}
