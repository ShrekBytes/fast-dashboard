...
**Pre-built image:** `ghcr.io/shrekbytes/fast-dashboard:latest` (see [Deploy](#deploy)).
...
# docker pull ghcr.io/shrekbytes/fast-dashboard:latest
# docker run -d -p 8080:8080 -v "$(pwd)/config:/app/config:ro" ghcr.io/shrekbytes/fast-dashboard:latest
```
...
Copy the Quadlet file to `~/.config/containers/systemd/`. It uses `Image=ghcr.io/shrekbytes/fast-dashboard:latest` and `AutoUpdate=registry`. Ensure config exists at the paths in the file (default: `%h/self-hosted/fast-dashboard/config` and `assets`). Uses `Network=host` — set `server.host` and `server.port` in `config.yml` to match. Optional: `EnvironmentFile` for env vars; uncomment the Docker socket volume for the container widget.
...
The image **ghcr.io/shrekbytes/fast-dashboard:latest** is built and pushed automatically via GitHub Actions (`.github/workflows/docker-publish.yml`):

- **On push to `main`:** build and push `ghcr.io/shrekbytes/fast-dashboard:latest`
- **On release (published):** push the same image with tags `vX.Y.Z` and `X.Y`

No extra secrets are required; the workflow uses the repo `GITHUB_TOKEN` with `packages: write`.
